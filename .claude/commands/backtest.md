...
```bash
cd python
py run_backtest.py --market jp --symbol 7203                                    # 単一期間
py run_backtest.py --market jp --symbol 7203 --walk-forward --n-splits 5       # Walk-Forward
py run_backtest.py --market us --symbol AAPL --model-type LightGBMModel --threshold 0.005 --source raw
```
...
| `api` | yfinance から直接取得 | DB未登録銘柄のテスト |
...
- `run_backtest_walk_forward(...)` — Walk-Forward バックテスト
