...
7. PR 作成後、`gh pr checks <PR番号> --repo <owner/repo>` で CI チェックの状態を確認する
   - `validate-pr-body` が失敗している場合は PR ボディを修正する（`gh pr edit` で4つの必須セクションを追加）
   - その他の CI が失敗している場合は原因を調査し、impl エージェントで修正してから再確認する
   - 全チェックが passed または pending になるまでこのステップを繰り返す
...
- PR 作成後は必ず CI チェックを確認し、`validate-pr-body` を含む全チェックが通過していることを確認してからユーザーに通知する
