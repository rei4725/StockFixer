...
うち NF-303-1（パイプライン連続失敗）については 2026-08-01 時点の本番 `results/scheduler_queue_state.json` を実際のロジックで評価し、**連続失敗数 0**（`daily_pipeline` の記録 91 件、直近は 2026-07-31 の success）であることを確認済みである。PostgreSQL 移行とコンテナ再作成を経た残骸による初日誤発報の懸念はない。
