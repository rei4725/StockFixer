...
純関数として実装する。入力は「要求したモデル名」「ロードできたモデル名」「今回の予測結果」「前回ラン統計」の4つのみ。DB もネットワークも触れないため、単体テストがモック地獄にならない。

要求リストとロード成功リストの両方を受け取る点が重要である。A-1 は「要求 vs ロード成功」を、A-2 は「ロード成功 vs 推論成功」を見る。仮に A-2 の期待値だけを持たせると、モデルファイルが1つ消えた場合に `loaded=1` / 各銘柄 `model_count=1` となって `1 < 1` が偽になり、縮退が完全に無言化する。この穴を塞ぐのが A-1 であり、そのためには要求リストが必要になる。#615 の教訓（テストは通るが実物では動かない / モックが実物と食い違っていた）を踏まないための選択である。
...
もう一つの呼び出し元 `predict_all_unified_multi_horizon`（`prediction_pipeline.py:162`）はホライズン `[1,3,5,10]` を跨いだモデル名リストを渡すが、**本番経路からは呼ばれていない**（呼び出し元は `tests/unit/test_prediction_pipeline.py` の4箇所のみ。2026-08-01 時点で確認済み）。したがって `model_count` の意味が異なる経路に単一の A-2 閾値が適用される懸念はない。
...
| ID | 条件 | 違反判定 | 根拠 |
|---|---|---|---|
| A-1 モデルロード欠損 | 要求した `model_types`（`prediction_pipeline.py:134` の `["UnifiedStockXGBoost", "UnifiedStockLightGBM"]`）とロードに成功したモデル名の差 | ロード数 < 要求数 なら違反 | モデルファイルの欠損・破損は議論の余地なく異常。かつ A-2 では原理的に検出できない（期待値がロード数から導かれるため、ロード自体が減ると閾値も一緒に下がってしまう） |
...
配線により実際に機能するのは **NF-303-1 と NF-303-5 の2本のみ**である。

NF-303-2（日次損失上限連続発動）・NF-303-3（ドリフト連続 Warn）・NF-303-4（/health
degraded 継続）の3本は、ストリークを更新する関数（`update_loss_limit_streak` /
`update_drift_warn_streak` / `update_health_degraded_streak`）に本番の呼び出し元が
存在しない（該当するのは `tests/unit/test_alert_service.py` のテストコードのみ）。
ストリークが一切書き込まれないため `_get_streak` は永久に 0 を返し、この3本は評価は
されるが `triggered=True` には原理的にならない。これらの updater を実際のイベント
発生箇所（自動発注のR-003判定・日次ドリフトチェック・ヘルスチェック実行後）へ配線
するのは本ブランチのスコープ外であり、別途対応が必要である。

初日誤発報の観点ではむしろ安全側である。ストリークが常に 0 を返す3本は必ず
`triggered=False` になるため、本ブランチの配線だけで初日から誤発報することはない。

実際に機能する NF-303-1（パイプライン連続失敗）については 2026-08-01 時点の本番
`results/scheduler_queue_state.json` を実際のロジックで評価し、**連続失敗数 0**
（`daily_pipeline` の記録 91 件、直近は 2026-07-31 の success）であることを確認済み
である。PostgreSQL 移行とコンテナ再作成を経た残骸による初日誤発報の懸念はない。
...
`src/prediction/predict_unified.py` の `get_cached_model` は、モデルファイルが見つからない場合（`FileNotFoundError`）に `None` をプロセス内キャッシュ（`_model_cache`）へ**永続的に**格納する。本番コードにこのキャッシュを clear する経路は存在しない。`_model_cache.clear()` を呼んでいるのはテストコードのみであり、本番の実行パスでは一度 `None` が入ったエントリはプロセスが再起動するまで書き換わらない。

スケジューラは常駐プロセスであるため、この挙動は運用上2つの非対称な結果をもたらす。**コールドスタート時にモデルが存在しなければ、以後そのプロセスが生きている限り A-1 が鳴り続ける**（キャッシュに `None` が固定されるため）。逆に**プロセス起動後にモデルファイルを削除しても、既にキャッシュ済みのモデルはメモリ上に残るため検知されない**。

これは本機能（出力 invariant チェック）が持ち込んだ回帰ではなく、`get_cached_model` の既存の挙動である。しかし `preload_models()` の戻り値が A-1 / A-2 の基準値として使われるようになったことで、このキャッシュの固定化が運用上の意味を持つようになった。週次再デプロイでプロセスが再起動する現在の運用であれば、キャッシュが古いまま残り続ける期間は最大1週間に限られるため実害は薄い。
