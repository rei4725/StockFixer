...
### champion_sharpe は下がる（想定内）

`champion_sharpe = max(control_sharpes)` であり、**最も汚染されていた対照群がチャンピオンだった**。IS 2024-07-25〜2026-07-25 の実測:

| control | 閾値≥1（現行） | 閾値≥3（新方式） |
|---|---|---|
| rsi_contrarian | **1.083** | 0.699 |
| bollinger_band | 0.782 | **0.782** |
| volatility_breakout | 0.559 | 0.559 |
| macd_rsi | 0.152 | 0.152 |
| ema_momentum | -0.047 | -0.047 |
| volume_breakout | -0.658 | -0.461 |

チャンピオンは rsi_contrarian（1.083）から bollinger_band（0.782）に交代し、対チャンピオン改善ゲートの基準は約 28% 下がる。1.083 が偽の数値だったのだから**この低下は正しい**。
