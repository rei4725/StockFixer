...
### 2.0. 時刻方針
- システム内部で保持・保存・比較する日時は、原則として UTC の timezone-aware datetime を使用する。
- ユーザー向けの通知、Discord 表示、運用画面、レポート本文に出す日時文字列は、必ず日本時間に変換して表示する。
- 日本時間の表示変換は `python/src/utils/japan_time.py` の共通部品を使用し、画面側・通知側で独自に `strftime` しない。
- スケジューラの実行判定は業務要件に合わせて Asia/Tokyo を使用してよいが、実行ログの保存時刻は UTC を優先する。
