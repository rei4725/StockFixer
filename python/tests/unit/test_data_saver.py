...
class TestSaveRawOhlcv(unittest.TestCase):
    ...
    def setUp(self):
        self.mock_upsert = patch("src.data.data_saver.upsert_raw_ohlcv", return_value=5).start()
...
class TestSaveRawStockData(unittest.TestCase):
    ...
    def setUp(self):
        self.mock_upsert = patch("src.data.data_saver.upsert_stock_features").start()
        self.mock_get_ticker = patch(
            "src.data.data_saver.get_ticker", side_effect=lambda m, s: s
        ).start()
        self.addCleanup(patch.stopall)
    ...
    def test_returns_none_when_get_stock_data_returns_none(self):
        ...
        with patch("src.data.data_saver.get_stock_data", return_value=None):
    ...
    def test_returns_none_when_get_stock_data_returns_empty(self):
        ...
        with patch("src.data.data_saver.get_stock_data", return_value=pd.DataFrame()):
            result = save_raw_stock_data("us", "AAPL", "2026-01-01", "2026-03-01")
        self.assertIsNone(result)
    ...
    def test_auto_detect_dates_when_none_given(self):
        ...
        df = self._make_df()
        with patch("src.data.data_saver.get_stock_data", return_value=df) as mock_gsd:
    ...
    def test_exception_during_auto_detect_returns_none(self):
        ...
        with patch("src.data.data_saver.get_stock_data", side_effect=Exception("network error")):
    ...
    def test_normal_path_calls_upsert_stock_features(self):
        ...
        df = self._make_df()
        with patch("src.data.data_saver.get_stock_data", return_value=df):
