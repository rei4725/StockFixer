...
class TestLoadFeatureData(unittest.TestCase):
    ...
    @patch("src.models.predict_unified.load_stock_features")
    def test_returns_dataframe_on_success(self, mock_load):
        """正常時に DataFrame が返ること"""
        from src.models.predict_unified import load_feature_data
...
class TestGetCachedModel(unittest.TestCase):
    ...
    def setUp(self):
        """テスト前にキャッシュをクリア"""
        import src.models.predict_unified as mod
...
class TestPreloadModels(unittest.TestCase):
    ...
    @patch("src.models.predict_unified.get_cached_model")
    def test_preloads_default_models(self, mock_get):
        """デフォルトモデルが事前ロードされること"""
        from src.models.predict_unified import preload_models
