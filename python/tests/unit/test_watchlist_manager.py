...
class TestFetchIndexSymbols(unittest.TestCase):
    ...
    @patch("src.watchlist.manager.fetch_sp500_symbols")
    def test_us_market_calls_sp500(self, mock_sp500):
        """us マーケット指定で fetch_sp500_symbols が呼ばれること"""
        from src.watchlist.manager import fetch_index_symbols
    ...
    @patch("src.watchlist.manager.fetch_nikkei225_symbols")
...
class TestVerifySymbolActive(unittest.TestCase):
    ...
    def test_active_symbol_returns_true(self, mock_ticker_cls):
        """有効な銘柄で True が返ること"""
        from src.watchlist.manager import verify_symbol_active
...
class TestDiffWatchlist(unittest.TestCase):
    ...
    @patch("src.watchlist.manager.verify_symbol_active")
    def test_new_symbols_are_added(self, mock_verify):
        """新規銘柄が added に追加されること"""
        from src.watchlist.manager import diff_watchlist
...
class TestRunWatchlistRefresh(unittest.TestCase):
    ...
    @patch("src.watchlist.manager.diff_watchlist")
    ...
    def test_runs_for_all_markets(
        ...
        from src.watchlist.manager import WatchlistDiff, run_watchlist_refresh
