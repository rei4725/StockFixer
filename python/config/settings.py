...
純粋なON/OFFの機能フラグ（真偽値）は `config/feature_flags.py` に分離している。
安全性に直結する数値パラメータ・秘密情報（APIキー・トークン）はこのファイル側に残す。
"""
...
class Settings(BaseSettings):
    ...
    # ---------- レジームレバレッジ戦略ペーパートレード（trading/regime_leverage_strategy） ----------
...
ENABLE_SHORT_SIDE: bool = flags.ENABLE_SHORT_SIDE
...
CLAUDE_TRADER_ENABLED: bool = flags.CLAUDE_TRADER_ENABLED
...
LLM_REVIEW_ENABLED: bool = flags.LLM_REVIEW_ENABLED
LLM_REVIEW_MODEL: str = settings.LLM_REVIEW_MODEL
LLM_REVIEW_MAX_TOKENS: int = settings.LLM_REVIEW_MAX_TOKENS

BACKTEST_REVIEW_ENABLED: bool = flags.BACKTEST_REVIEW_ENABLED
...
FACTORY_HYPOTHESIS_REVIEW_ENABLED: bool = flags.FACTORY_HYPOTHESIS_REVIEW_ENABLED
...
FACTORY_CLAUDE_RULEGEN_ENABLED: bool = flags.FACTORY_CLAUDE_RULEGEN_ENABLED
...
STRATEGY_PROMOTION_CHECK_ENABLED: bool = flags.STRATEGY_PROMOTION_CHECK_ENABLED

TEST_GAP_ENABLED: bool = flags.TEST_GAP_ENABLED
...
AUTO_PROMOTE_MODEL: bool = flags.AUTO_PROMOTE_MODEL
...
DB_COMPACT_ENABLED: bool = flags.DB_COMPACT_ENABLED
...
FACTORY_ENABLED: bool = flags.FACTORY_ENABLED
