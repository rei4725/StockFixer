    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            is_rate_limit = any(keyword in error_msg for keyword in [
                "rate limit", "too many requests", "429", "430",
                "retry", "temporarily unavailable", "timeout"
            ])
            is_network = any(keyword in error_msg for keyword in [
                "connection", "timeout", "reset", "connection refused",
            ...
            if attempt == max_retries or not is_retriable:
                logger.error(
                    f"[リトライ失敗] ({attempt}/{max_retries}) {type(e).__name__}: {e}"
                )
                raise
