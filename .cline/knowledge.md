...
## バッチデータ生成・既存csv削除のTips
- batch_run_data_creation.pyはmarket/symbolのみrun_data_creation.pyに渡すことでシンプルな設計にできる。
- save_stock_data_with_featuresでstart_date, end_dateを省略可能にし、未指定時は「銘柄ごとの最古日付」「バッチ実行日時」を自動決定できる。
- データ保存前に同一ディレクトリ内の既存csvファイルを全削除することで、常に最新データのみを保持できる。
- データの一貫性・最新化が担保でき、運用・保守が容易になる。
...
## モデル保存ディレクトリ設計・運用Tips
- モデル保存先は `python/models/[market]_[symbol]/モデル名.joblib` 形式とし、銘柄ごとにサブディレクトリを分割することで管理性・保守性を向上させる。
- サブディレクトリ名（例: us_AAPL）から market, symbol を抽出し、モデル保存・ロード時に利用する。
- run_model_creation.py では python/data 配下のサブディレクトリも含めて csv ファイルを再帰的に探索し、全銘柄分のモデルを自動作成できるようにする。
- glob.glob の recursive=True オプションを活用し、`os.path.join(data_dir, "**", "*.csv")` で全csvを取得する。
- データディレクトリや保存先パスは絶対パスで解決し、実行環境依存のバグを防止する。
